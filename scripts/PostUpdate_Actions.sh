...
if [[ $Node_bin_ver_NUM -ge $Chng_Config_ver ]] && \
   [[ $Node_bin_ver_NUM -ne $Node_SVC_ver_NUM ]];then
    ...
    "${SCRIPT_DIR}/Send_msg_toTelBot.sh" "$HOSTNAME Server" "$Tg_CheckMark INFO: Node updated. Service restarted. Current versions: node ver: ${EverNode_Version} node commit: ${Node_bin_commit}, console - ${Console_Version}, ever-cli - ${CLI_Version}" > /dev/null 2>&1
